...
# - Dynamic gating (same as your working filter)
# - Per-pair gate: χ²_2d (default 11.83 ~ 99% for 2 dof)
# - Joint gate: χ²_{2k} at joint_sig (default 0.99) via a small lookup table
# - No color/side constraints; no CSVs/RViz; prints only.
#
# Run:
#   ros2 run fslam_da jcbb
#
...
# ---- small χ² quantile table at 0.99 for dof=2..40 (rounded) ----
# source values (common tables); enough for k up to 20 matches (df=40)
CHI2_0p99 = {
...
def chi2_thresh(dof: int, joint_sig: float) -> float:
    # we only support 0.99 here; if user changes joint_sig, fall back conservative
    if abs(joint_sig - 0.99) < 1e-3:
        return CHI2_0p99.get(dof, CHI2_0p99[max(k for k in CHI2_0p99 if k <= dof)])
    # conservative fallback: scale roughly linearly
    base = CHI2_0p99.get(dof, CHI2_0p99[max(k for k in CHI2_0p99 if k <= dof)])
    return base  # keep it simple; user defaults are 0.99 anyway
...
class JCBBNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"JCBB up. Map cones: {self.MAP.shape[0]}")
    ...
    def cb_cones(self, msg: ConeArrayWithCovariance):
        ...
        # Candidates (same gating as filter)
        crop2 = self.map_crop_m**2
        MAP = self.MAP
        cands: List[List[Tuple[int,float]]] = []  # per-obs: [(map_id, m2),...]
        total_cands=0
        for pw, S0, pb in zip(obs_pw, obs_S0, obs_pb):
            ...
            if not cc and self.min_candidates>0 and near.size>0:
                # optional Euclid fallback
                k = min(self.min_candidates, near.size)
                by = sorted([(int(mid), d2[mid]) for mid in near], key=lambda t:t[1])[:k]
                # mark as "weak" by setting m2 slightly above gate (still considered in JCBB; joint gate will reject if needed)
        ...
        if total_cands==0:
            # nothing to associate this frame
            self._print_line(t_z, dt_pose, v, n_obs, 0, total_cands, 0, [])
            self.frame_idx += 1
            return

        # Reorder obs by ascending candidate count (helps pruning)
        order = sorted(range(n_obs), key=lambda i: len(cands[i]) if cands[i] else 9999)
        rev = np.zeros(n_obs, dtype=int)
        for pos,i in enumerate(order): rev[i]=pos
        cands_ord = [cands[i] for i in order]

        # JCBB search
        best_pairs: List[Tuple[int,int,float]] = []  # [(obs_idx_orig, map_id, m2), ...]
        best_K = 0

        # map usage (no duplicate map points)
        used = set()

        # cumulative sum of m2 (independent block-diag assumption)
        def dfs(idx, cur_pairs, cur_sum):
            nonlocal best_pairs, best_K
            # bound: even if we match all remaining, max matches = cur + (n-idx)
            cur_K = len(cur_pairs)
            if cur_K + (len(cands_ord)-idx) < best_K:
                return

            # joint gate check on current set (df=2*cur_K)
            if cur_K>0:
                df = 2*cur_K
            ...
            # Option 1: skip this obs (unmatched)
            dfs(idx+1, cur_pairs, cur_sum)

            # Option 2: try candidates
            for (mid, m2) in cands_ord[idx]:
                if mid in used:
                    continue
                # per-pair already gated, add and continue
                used.add(mid)
    ...
    def _print_line(self, t_z, dt_pose, v, n_obs, n_match, n_cands_total, proc_ms, show_pairs):
        ...
        base = (f"[jcbb] pass_dt={int(pass_dt*1000)}ms  Δt_pose={int(dt_pose*1000)}ms  "
